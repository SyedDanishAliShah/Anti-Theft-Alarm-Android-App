...
class AntiPocketDetectAlarmActivity  : AppCompatActivity(), SensorEventListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_anti_pocket_detect_alarm)
        ...
        backIcon = findViewById(R.id.back_icon_3_anti_pocket)
        animationView = findViewById(R.id.animation_view_anti_pocket)
        activateCircle = findViewById(R.id. anti_pocket_activation_circle)
        stopCircle = findViewById(R.id.motion_detection_activation_stop_circle_anti_pocket)
        stopAlarmWithPinCodeSwitch = inflatedLayout.findViewById(R.id.stop_alarm_with_pin_code_card_switch)
        flashCardView = findViewById(R.id.flash_and_vibration_card_for_flash_anti_pocket)
        vibrateCardView = findViewById(R.id.flash_and_vibration_card_for_vibration_anti_pocket)
        flashSwitch = findViewById(R.id.flash_light_switch_anti_pocket)
        vibrateSwitch = findViewById(R.id.vibration_switch_anti_pocket)
        pressActivateButtonTextView = findViewById(R.id.press_activate_button_tv_anti_pocket)
        activatingPleaseWaitTextView = findViewById(R.id.activating_please_wait_tv_anti_pocket)
        activateTextView = findViewById(R.id.activate_tv_anti_pocket)
        stopTextView = findViewById(R.id.stop_tv_anti_pocket)
...
//        redCircle = findViewById(R.id.red_circle_1)
        ...
        if (proximity == null) {
            ...
            Toast.makeText(this, "Device does not have a proximity sensor", Toast.LENGTH_SHORT)
                .show()
...
private fun startAnimation() {
    ...
    animationView.addAnimatorListener(object : Animator.AnimatorListener {
        ...
        override fun onAnimationEnd(p0: Animator) {
            ...
            sensorManager.registerListener(
                this@AntiPocketDetectAlarmActivity,
                proximity,
    ...
    override fun onSensorChanged(event: SensorEvent) {
        ...
        val proximityThreshold = 5.0f

        if (event.sensor.type == Sensor.TYPE_PROXIMITY && event.values[0] == proximityThreshold) {
            // The condition is met when the device is taken out of the pocket
            // Implement your logic here
            if (isAnimationCompleted && stopAlarmWithPinCodeSwitch.isChecked) {
                // Both conditions are true, handle the case here
                val intent = Intent(this@AntiPocketDetectAlarmActivity, StopAlarmWithPinCodeActivity::class.java)
                startActivity(intent)
                finish()
                startService()
            } else {
                val intent = Intent(this@AntiPocketDetectAlarmActivity, StopAlarmActivity::class.java)
                startActivity(intent)
                finish()
                startService()
            }
